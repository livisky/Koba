 ...
 *
 */
define("common/ysDialog/0.0.1/ysDialog-debug", [ "common/createClass/0.0.1/createClass-debug" ], function(require) {
    var createClass = require("common/createClass/0.0.1/createClass-debug"), Dialog = require("common/dialog/0.0.1/dialog-debug");
    var template = [ '<div class="sea-ysdialog">', '<h4 class="sea-ysdialog-title"></h4>', '<div class="sea-ysdialog-bd"></div>', '<a class="sea-ysdialog-close" href="javascript:;" title="点击关闭窗口">╳</a>', "</div>" ].join("");
    ...
    YsDialog.confirm = function(tipText, titleText, callback) {
        var dialog = YsDialog.confirm._dialog, dialogEle, confirmTip, closeBtn, confirmBtn, cancelBtn;
        if (!dialog) {
            dialog = new YsDialog({
                mask: true,
                titleText: "&nbsp;"
            ...
            dialog.append([ '<div style="padding: 10px 20px">', '<div class="J-confirmTip" style="padding:10px 0;line-height:1.5;text-align:center;"></div>', '<div style="height:28px;padding: 20px 0 10px">', '<div style="margin:0 auto;width: 145px;height: 28px;">', '<a class="sea-ysdialog-btn sea-ysdialog-btn-grey" href="javascript:;" style="margin-left:15px;float:right">取消</a>', '<a class="sea-ysdialog-btn sea-ysdialog-btn-orange" href="javascript:;" style="float:right;">确定</a>', "</div>", "</div>", "</div>" ].join(""));
        }
    ...
    YsDialog.popup = function(titleText, config, callback, initFn, standBy) {
        ...
        config.temp = [ '<div style="padding: 10px 10px;">', '<div class="confirmTip" id="content" style="padding:10px 0;line-height:1.5;text-align:center;"></div>', '<div style="height:28px;padding: 20px 0 10px;clear:both;">', '<div style="margin:0 auto;width: 145px;height: 28px;">', '<a class="sea-ysdialog-btn sea-ysdialog-btn-grey" href="javascript:;" style="margin-left:15px;float:right">' + config.cancel + "</a>", '<a class="sea-ysdialog-btn sea-ysdialog-btn-orange" href="javascript:;" style="float:right;">' + config.ok + "</a>", "</div>", "</div>", "</div>" ].join("");
        var dialog = YsDialog.popup._dialog, dialogEle, confirmTip, closeBtn, confirmBtn, cancelBtn;
        if (!dialog) {
            dialog = new YsDialog({
                mask: true,
                titleText: ""
    ...
    YsDialog.hint = function(conf) {
        var dialog = YsDialog.hint._dialog, dialogEle = dialog ? dialog.getAttr("widgetEle") : null, confirmTipEle, confirmBtn;
        conf = conf || {};
        if (!dialog) {
            dialog = new YsDialog();
            dialog.setSize(213);
            ...
            dialogEle = dialog.getAttr("widgetEle");
            dialog.append([ '<div style="padding: 10px 30px">', '<div class="J-confirmTip" style="padding:20px 0;color:red;line-height:1.5;"></div>', "</div>", "</div>" ].join(""));
        ...
        var height = $("body").prop("scrollHeight") || $("body").outerWidth(), width = $("body").prop("scrollWidth") || $("body").outerHeight(), newmak = $('<div style="height: ' + height + "px; width: " + width + 'px; opacity: 0.7; position: absolute; left: 0px; top: 0px; z-index: 999999; background: rgb(170, 170, 170);"></div>').appendTo("body");
        if (conf.timeout) {
            setTimeout(function() {
                dialog.hide();
                newmak.hide();
    ...
    YsDialog.alert = function(conf) {
        var dialog = YsDialog.alert._dialog, dialogEle = dialog ? dialog.getAttr("widgetEle") : null, confirmTipEle, confirmBtn;
        conf = conf || {};
        if (!dialog) {
            dialog = new YsDialog({
                mask: true,
            ...
            dialogEle = dialog.getAttr("widgetEle");
            dialog.append([ '<div style="padding: 10px 30px">', '<div class="J-confirmTip" style="padding:10px 0;line-height:1.5;"></div>', '<div style="height:28px;padding: 20px 0 10px">', '<a class="sea-ysdialog-btn sea-ysdialog-btn-orange" href="javascript:;" style="float:right;">确定</a>', "</div>", "</div>" ].join(""));
